...
- T1–T15 전부 완료, 보류 0건. 최종 `pnpm verify` exit 0(클린 트리). 커밋 15개(bc4e666→ae14deb, feat/guide-alignment), 가이드 문서 무수정 원칙 준수.
- P0·P1: 계약 파일 사고 복구(재방출·바이트 동일 증명), drift 게이트의 untracked 허점 봉합(fail-closed), CI 워크플로(mise 정본·로컬=CI 동일 명령)·oasdiff 게이트(digest 핀·skip-pass 의미론) 신설.
- P2: presentation→web.v1.post 슬라이스 정렬(계약 불변 증명), test-architecture 모듈(클래스패스 파생·closure fail-closed·base finder deletedAt 예외+픽스처 상시 검증), OpenAPI 애노테이션+강제 규칙 2종(freeze로 과도기 관리 후 자기청산), 페이지네이션 공통화·1-based 전환(의도적 breaking 절차 수립), app-migration 분리(부팅 Flyway 소멸 — compose 승격 포함), 경계 장치 전 구성 검사·api 최소화, 전 모듈 루트 표식.
- P3: 게이트 중앙 패키지(@board/eslint-config·tsconfig — turbo 캐시 무효화 개선 동반), boundaries v7 전환(deep import·no-unknown 강제, 경고 0), server-only 직접 가드·no-orphans error, 타입 정직성(UUID 브랜드·int64 계약 string 표현).
- 잔여·후속 후보(비차단, 각 작업 로그에 상세): codegen clean-regeneration / 브랜치 보호·required check(리포 설정) / .mise.toml 정밀 핀 / common-jpa·common-web api 표면 축소 / 정본 문서 정비 후보(sharing.md int64 비고·architecture.md 53행·백엔드 135행 "등록" 문구) / server-only 상시 게이트(build)의 verify·CI 미포함 비대칭 / 원격 CI 첫 실행 확인(push 필요).
