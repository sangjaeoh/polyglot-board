...
- 계약 파일은 `apps/backend/docs/openapi/`에 둔다. 현재 방출 단위는 `app-api` 하나이고 파일명은 `openapi.json`이다.
  - 방출 단위가 복수가 되면 파일명은 방출 단위 이름을 따른다. 단일 계약인 동안 선제 개명은 하지 않기로 했다.
...
- 파괴적 변경(삭제·의미변경)을 분류하는 검사(oasdiff)는 미도입이다. 계약 변경 자체는 drift 게이트가 표면화하고 파괴성 판단은 리뷰가 한다.
