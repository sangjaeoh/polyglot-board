...
- 새 모듈은 `module-{layer}/{prefix}-{name}` 디렉터리에 `build.gradle.kts` 하나(해당 유형 컨벤션 플러그인만 적용)로 만들고 settings에 등록한다.
  - 예: `module-domains/domain-order/build.gradle.kts`(`convention.domain-module`만 적용). 래퍼·플러그인 구현 등 빌드 하네스 자체는 이미 구성된 것으로 전제한다.
...
- 도메인마다 PostgreSQL 스키마 하나를 소유한다.
  ...
  - 새 도메인은 `db/migration/{name}/`에 스키마 생성 + 테이블 DDL을 두고, 실행 앱의 `FlywayConfig`가 `FlywayConfigurationCustomizer`로 `SchemaFlyway.applySchema`(common-jpa)를 그 스키마에 적용해 부팅 시 마이그레이션한다. 누락하면 빌드는 green인데 `ddl-auto=validate` 기동이 스키마 불일치로 실패한다.
...
- 공용 코드는 아래 역할 표에 맞는 common 모듈에 둔다. 도메인 로직·도메인 지식은 어느 common에도 두지 않는다.
  ...
  | `common-jpa`       | JPA 공통 지원 — Auditing·`SchemaFlyway`                                        |
...
- 데이터 반환은 내부는 엔티티, 경계는 Info, 명령은 ID다.
  ...
  - 예외: 생성 명령은 생성 결과 Info 반환을 허용한다. 응답 조립을 위한 재조회가 두 번째 트랜잭션을 열고 그 사이 삭제 레이스로 방금 만든 자원에 404가 나는 것을 막는다. 엔티티 비노출이라는 취지는 Info 반환으로도 지켜진다.
