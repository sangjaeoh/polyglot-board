...
- `BaseException`과 `ErrorCode` 인터페이스는 common-core가 소유한다.
...
- 도메인 용어집은 backend 루트 `REQUIREMENTS.md`의 용어 표가 소유하고, 필드 수준 상세는 `DOMAIN_MODEL.md`가 소유한다.
...
- 역할 접미사 이름은 provided 계약 인터페이스가 소유한다.
- 구현 서비스는 `Default{계약명}`으로 짓는다.
