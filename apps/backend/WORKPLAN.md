...
- T3 완료 — 3구역 재편(domain·application/provided·required·info, provided 인터페이스 4종 + package-private Default* 구현) + 아키텍처 테스트 정렬(구역 마커 파생, JPA 애노테이션 기반 비노출, 구역 의존 방향·adapter 비노출·provided/info만 소비·시그니처 JPA 금지·구현 package-private 신설). 판정: info의 정적 `from(entity)` 팩토리는 coding-conventions 규정 패턴이라 시그니처 규칙에서 명시 예외(모듈 밖은 엔티티 임포트 불가로 호출 불가). 커밋: 5a0d10b
- T4 완료 — `PostAppender.register`가 UUID만 반환(명령 최소 결과), createPost는 getPost 재조회로 201 조립(SELECT 1회 추가 수용), `BoardFacade.update`→`edit`·`BoardController.updatePost`→`editPost` 개명. 판정: operationId는 계약이 소유하는 공표 식별자라 `updatePost`로 고정(pin) — sharing.md 단방향 소비·파괴적 변경 금지와 T4의 계약 불변 전제를 동시에 만족하는 유일 구현. openapi.json 무변경으로 drift 게이트 통과 확인. 커밋: dae334c
- T5 완료 — 시나리오 1-23 전부 `@DisplayName` 1:1 구현(14-16의 `/` 문장은 두 테스트로 분리). 리포지토리 `@DataJpaTest` 전환, 서비스 4종 목 제거·실 DB 슬라이스(Appender 신설), 웹 슬라이스 신설(17-21), 통합은 22-23으로 축소(삭제 6종은 단위·웹 슬라이스로 이관). 배선: common-web 테스트 의존 + Boot 4 분리 아티팩트 `spring-boot-data-jpa-test`를 domain-module 컨벤션에 추가. 판정: `@EnableJpaRepositories`·`@EntityScan`을 앱 진입점에서 package-private `JpaConfig`로 분리(웹 슬라이스가 JPA를 끌어오지 않게 — 공개 표면 확장 없음, 앱 동작 동일). 커밋: (본 커밋 — 다음 태스크에서 해시 기입)
