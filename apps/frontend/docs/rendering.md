...
- 렌더 경계는 서버/클라이언트 계약 경계다.
- 모든 `'use client'`는 번들 비용·직렬화 경계·시크릿 노출 표면을 만든다.
- 클라이언트 props는 egress 검증을 통과한 데이터에서 파생한 직렬화 가능·최소 계약만 전달한다.
- 함수·클래스 인스턴스·서버 전용 객체를 클라이언트 props로 전달하지 않는다.
- 클라이언트 뷰모델은 → [data](data.md)의 read-model 규칙을 따른다.
- 시크릿 경계는 → [architecture](architecture.md)의 서버 경계를 따른다.
...
- 라우팅 조립 구조는 → [architecture](architecture.md)의 app 라우팅 구조를 따른다.
...
- 인터셉트 라우트(`(.)`·`(..)`)는 목록 위 모달 상세처럼 컨텍스트를 유지하는 화면 전환에 사용한다.
...
- 가로채기는 최후 수단이다.
- 인가는 라우트·route handler·Server Action 내부에서 우선 판단한다(→ [data](data.md)의 Server Action).
...
- 에러 계약은 → [data](data.md)의 에러 처리를 따른다.
...
- 루트 레이아웃에 `metadataBase`를 설정한다.
- 이를 통해 OG·canonical 상대 URL 오류를 방지한다.
...
- 비-fetch 메모이즈는 → [data](data.md)의 데이터 흐름을 따른다.
